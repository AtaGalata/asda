...
setInterval(async () => {
    let guild = client.guilds.cache.get('guild id gir');
    if (!guild) return console.log('Guild ID\'si Belirtilmemiş'.red);
    let role = guild.roles.cache.get('rol id gir');
    if (!role) return console.log('Role ID\'si Belirtilmemiş'.red);
    let channel = guild.channels.cache.get('log kanalı id gir');
    ...
    await guild.members.cache.map(async m => {
        ...
        let control = status.includes('DURUMU NE OLANLARA ROL VERİLİCEK');
